...
static int battle_calc_attack_skill_ratio(struct Damage* wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	switch(skill_id) {
		...
		case MC_MAMMONITE:
			...
			break;
		case HT_POWER:
			skillratio += 50 + 20 * skill_lv + (2 * sstatus->dex) + (5 * (pc_checkskill(sd, BS_SKINTEMPER)));
			...
			if (sc && sc->data[SC_POEMBRAGI])
				skillratio += 10 + 2 * sstatus->agi;
			if (sc && sc->data[SC_ASSNCROS])
				skillratio += 10 + 2 * sstatus->str;
			if (sc && sc->data[SC_MANU_ATK])
				skillratio += 10 + (0.2 * sstatus->str) + (0.2 * sstatus->agi) + (0.2 * sstatus->vit) + (0.2 * sstatus->int_) + (0.2 * sstatus->dex) + (0.2 * sstatus->luk);
			if (sc && sc->data[SC_MANU_DEF])
		...
		case NJ_SYURIKEN:
			...
			if (sd->inventory_data[sd->equip_index[EQI_AMMO]]) {
				if (sd->inventory_data[sd->equip_index[EQI_AMMO]]->nameid == 13249) {
					skillratio += 100 + (0.2 * sstatus->str) + (0.2 * sstatus->agi) + (0.2 * sstatus->vit) + (0.2 * sstatus->int_) + (0.2 * sstatus->dex) + (0.2 * sstatus->luk);;
				}
				...
				if (sd->inventory_data[sd->equip_index[EQI_AMMO]]->nameid == 13251) {
					skillratio += 3 * ((status_get_max_hp(src) - status_get_hp(src)) * 100) / status_get_max_hp(src);
				}
				...
				if (sd->inventory_data[sd->equip_index[EQI_AMMO]]->nameid == 13247) {
					skillratio += -50 + 2 * sstatus->luk;
				}
				if (sd->inventory_data[sd->equip_index[EQI_AMMO]]->nameid == 13248) {
					skillratio += 100 + 2 * sstatus->dex;
				}
			}
			break;
...
static void battle_calc_defense_reduction(struct Damage* wd, struct block_list *src,struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
	...
	int i = 0;
	if (sd) {
		i += sd->indexed_bonus.ignore_def_by_race[tstatus->race] + sd->indexed_bonus.ignore_def_by_race[RC_ALL];
		i += sd->indexed_bonus.ignore_def_by_class[tstatus->class_] + sd->indexed_bonus.ignore_def_by_class[CLASS_ALL];

	}

	if (sc && sc->data[SC_EXPIATIO]) {
		i += 1 * sc->data[SC_EXPIATIO]->val1; // Changed to 1% per level

	}
	if (sc && sc->data[SC_ASSNCROS]) {
		i += 2 * sc->data[SC_ASSNCROS]->val1; // Changed to 1% per level
	...
	if (tsc) {
		...
		if( tsc->data[SC_CAMOUFLAGE] ){
			i += 9 * tsc->data[SC_CAMOUFLAGE]->val3; //5% per second
...
static void battle_calc_attack_plant(struct Damage* wd, struct block_list *src,struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
    ...
    if( attack_hits || wd->damage > 0 ){
		wd->damage = 1;
		}//In some cases, right hand no need to have a weapon to deal a damage
	if(skill_id != 0){
		std::shared_ptr<s_skill_db> skill = skill_db.find(skill_id);
		if (attack_hits && skill->num[0] < 0) 
			wd->damage = (skill->num[0]) * (-1);
	}
				...
				break;
					...
					case BA_MUSICALSTRIKE:
						...
						if (tsc && tsc->data[SC_BURNING])
							skillratio += 1 * (sstatus->int_);
						if (sc && sc->data[SC_MANU_DEF])
							skillratio += 10 * skill_lv + 1 * (sstatus->int_);
						if (sc && sc->data[SC_CONCENTRATE])
							skillratio *= 1 + 12 * (1 - (3000 + status_get_max_sp(src) - status_get_sp(src)) / (3000 + 1.1 * (status_get_max_sp(src) - status_get_sp(src))));
						if (sc && sc->data[SC_DEATHBOUND])
							skillratio *= 1 + 24 * (1 - (3000 + status_get_sp(src)) / (3000 + 1.1 * status_get_sp(src)));
